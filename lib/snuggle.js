                ...
                temp.open({"suffix": "." + processor[1]}, function (err, tempInfo) {
                    if (err) {
                       throw "Could not open tempfile for file '" + filePath + "'. Error: " + err;
                        ...
                        commandRunner.runCommand(processor[0], filePath, tempInfo.path, function () {
...
var main = function(staticConfig) {
    ...
    var numWorkers = 4,
        fileQueue = new SimpleQueue(handleFile, function (err, result, filePath) {}, undefined, numWorkers),
